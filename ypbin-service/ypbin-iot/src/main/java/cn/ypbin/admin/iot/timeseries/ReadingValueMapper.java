...
public final class ReadingValueMapper {
     ...
     *
     * @param column       目标列名
     * @param numericValue 数值；**文本行必须为 {@code null}**（用原始 `double` 会让"永不双写"易被违反）
    ...
    public static MappedValue map(String raw) {
        ...
        if (value.isEmpty()) {
            ...
            return new MappedValue(COLUMN_TEXT, null, value);
        }
        if ("true".equalsIgnoreCase(value) || "false".equalsIgnoreCase(value)) {
            return new MappedValue(COLUMN_TEXT, null, value.toLowerCase(Locale.ROOT));
        }
        boolean integerForm = looksLikeInteger(value);
        if (integerForm && hasLeadingZero(value)) {
            // 前导零的整数（如 "007"）通常是**编码/序列号**：落数值列会丢形态 ⇒ 文本列
            return new MappedValue(COLUMN_TEXT, null, value);
        }
        if (integerForm) {
            ...
            // 用区间判定而不是 Math.abs：Math.abs(Long.MIN_VALUE) 仍为负 ⇒ 会误判为「可精确表示」
            if (exactLong >= -MAX_EXACT_LONG_IN_DOUBLE && exactLong <= MAX_EXACT_LONG_IN_DOUBLE) {
        ...
        if (looksLikeDecimal(value)) {
            try {
                ...
                // 形态像小数但解析失败（如 "1.2.3"）：按文本处理（不做静默兜底转换）
                return new MappedValue(COLUMN_TEXT, null, value);
            }
        }
        return new MappedValue(COLUMN_TEXT, null, value);
    ...
    private static boolean looksLikeInteger(String value) {
        ...
        for (int i = index; i < value.length(); i++) {
            ...
            if (ch < '0' || ch > '9') {
                // 只认 ASCII 数字：全角数字（如 "１２３"）不是协议侧的数据形态，且与数值解析路径口径不一
                return false;
            }
        }
        return true;
    ...
    private static boolean hasLeadingZero(String value) {
        int index = startsWithSign(value) ? 1 : 0;
        return value.length() - index > 1 && value.charAt(index) == '0';
    }
