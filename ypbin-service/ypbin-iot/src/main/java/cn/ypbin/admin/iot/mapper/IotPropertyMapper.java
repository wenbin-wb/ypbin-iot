 ...
 * @since 2026-09-20
 */
public interface IotPropertyMapper extends BaseMapper<IotProperty> {

    /**
     * 物理删除（绕过逻辑删除）：物模型结构是「当前草稿」的派生数据，全量替换时必须真正移除。
     *
     * <p>为什么不能沿用逻辑删除：iot_property 上有<b>业务唯一键</b>且不含 is_deleted
     ...
     * @return 影响行数
     */
    @Delete("<script>DELETE FROM iot_property WHERE service_id IN "
     ...
     *
     * <p>逻辑删除行仍占用 iot_property 的业务唯一键，会与后续插入冲突；单条删除与全量替换
     ...
     * @return 影响行数
     */
    @Delete("<script>DELETE FROM iot_property WHERE id IN "
