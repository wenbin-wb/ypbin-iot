...
**因此**：
1. 本项作为「本轮新暴露」登记在此；
2. ~~是否维持「本轮不轮换」需由用户在知情后重新裁定~~ → **用户已于 2026-09-26 知情后裁定：维持不轮换**。
   理由（用户原文要点）：① 这两个值本来就对任何有 docker 权限的人可读（`docker inspect` 的 `Env`/`Cmd`），
   会话文本这次新增的是「受控流程内部可见」，**没有增加新的技术攻击面**；② 轮换要重建 mysql + redis
   并牵动全部连库/连缓存服务（含停写窗口），**收益不成比例**；③ 与本项目「脚手架/初始阶段不过度严格、
   正式部署时再收紧」的口径一致。**风险登记（§4）继续保留**：将来收紧 docker 权限或对外暴露时应轮换。
...
## 10. 硬规矩与教训（2026-09-26 本轮确立，**后续必须遵守**）

### 10.1 硬规矩

| 编号 | 规矩 | 由来 |
|---|---|---|
| **H1** | 采样 `docker events` **一律只输出计数或键名**：`grep -c` / `wc -l` / 布尔。**禁止打印 `Action` 字符串**，禁止 `--format '{{json .}}'` 落盘后整段回读。含明文的临时事件流**用完即删**（`umask 077`、600、700 目录） | 本轮事故：`Action` 字段**本身就内嵌 exec 命令行**（`exec_create: mysqladmin ping -h localhost -p<口令>`）。我按行替换了 JSON 正文里的口令值，却仍把 `Action` 直接打印 ⇒ 明文口令进了会话输出。脱敏只覆盖"我以为的字段"是不够的 |
| **H2** | 清理**只允许限定在本轮自己产生的路径内**；**跨轮次产物只能「登记 + 请示」，不得顺手删** | 本轮越界：清理 dry-run 备份时我一并删掉了 `/opt/ypbin/{secret-rotation,token-rotation}-*`（早前轮次的轮换回滚物）。用户裁定**结果可接受**（里面是已作废的旧值，销毁本身正确、也不该回滚到已泄露的值），但**流程属越界** ⇒ 立此规矩 |
| **H3** | 凭据类断言**必须与外部真值比对**，**禁止自比自**（`assert got == token` 在 `token` 被覆盖时恒真）；同名变量**跨用途必须显式区分命名** | 本轮缺陷：`fix-access-trusted-source.py` 里 `token` 一名两用（`GATEWAY_SIGN_TOKEN` 被 `login()` 的 JWT 覆盖）⇒ `build_block(token)` 会把 **JWT 写进 `ypbin-access.yaml`**，且断言自比自 + 打印假绿。由 R6 复核者桩化实证抓出；修复见 `tools/test_rotate_nacos_password.py::CredentialSeparationTest`（hermetic、修前必红） |

### 10.2 三条自查教训（都是本轮用血换来的）

1. **「宿主 `ps` 可读」是未经测量的断言**：我据此把 redis 旧写法的暴露面写成「`docker top`/宿主 `ps` 可读」，
   复核者实测**看不到** —— redis 启动后会改写进程标题为 `redis-server *:6379`，原始 argv 不再可见。
   **真实暴露面**是 `docker inspect .Config.Cmd`（持久）+ 健康检查 exec 子进程 argv 的毫秒级采样窗口 +
   `docker events` 的 `exec_create`/`exec_start`（完整）。⇒ **规则：暴露面结论必须来自实测，不能来自"应该能看到"**；
   且**采样 `docker top` 命中 0 不能当作"argv 干净"的判据**（实测 mysql 命中率约 1/3207、redis 为 0）。
2. **脱敏必须覆盖所有字段**（见 H1）：脱敏是按"我知道哪些字段有值"做的，漏一个字段就等于没脱敏。
   ⇒ **规则：宁可只输出计数/键名，不做"整段打印 + 文本替换"**。
3. **`token` 一名两用导致跨凭据写错**（见 H3）：同一函数里两个不同来源的凭据共用一个变量名，
   后赋值者静默覆盖前者，而自比自断言让错误"通过检验"。⇒ **规则：跨用途变量必须区分命名；
   凭据断言必须比外部真值**；并为此类缺陷补**能咬人的 hermetic 回归用例**。

### 10.3 本轮回滚物在哪里（**未删**，供随时回滚）

| 位置 | 内容 | 权限 |
|---|---|---|
| `/opt/ypbin/cred-hardening-20260926-153455/` | `deploy.env.bak`（改前 `.env`）、`docker-compose.yml.bak`、`install.sh.bak`、`old.fp`（新旧指纹）、`before.sha`、`rollback.sh`（一键：关回 auth → 回滚口令 → 重建 nacos/redis/服务，**不含明文**）、`regcount.sh`（只读注册数） | 目录 700；文件 600 / 脚本 700 |
| `/opt/ypbin/nacos-ypbin-access.yaml.bak-20260926-163306` | 本轮 dry-run 的 live 配置快照（**未做任何改动**，仅 `--apply` 才需要它） | 600 |
| `/opt/ypbin/iotdb-rotation-20260926-144415/` | **早前轮次**的 IoTDB 口令轮换回滚物 | 700/600（按 H2：只登记，不动） |

> 本轮自己的 `/tmp/access-live-*.yaml`（dry-run 产物，**该工具以默认 0644 写到 `/tmp`**）
> 已按 H2 在"本轮自己产生的路径内"清除，并**登记该工具缺陷**：`fix-access-trusted-source.py`
> 落 `live_path`/`new_path` 时未 `chmod 600`，与它自己的"中间文件一律 600"约定不符 ——
> **下一轮小批量修**（本轮已收口，不扩范围）。
