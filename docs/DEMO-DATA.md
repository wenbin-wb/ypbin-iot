...
## ⚠️ 现网状态变更声明（2026-09-26）

**`demo-dev-curve`（设备 9300012）现由 access 真实采集链路驱动，本文档中它的原定口径已失效，一律以现场为准。**

原因：2026-09-26 启用 access 采集链路并做了端到端验证（见 [`ACCESS-ENABLE.md`](./ACCESS-ENABLE.md)），
该设备的 `endpoint` 被指向测试用 TCP 模拟器，于是它的演示数据被**真实链路**改写：
...
1. **不要把 9300012 同时当作「演示曲线」和「真链路证据」**。若要恢复演示原口径，按本文档
   「清理手册」重置该设备，或把 `endpoint` 改回 `tcp://10.10.0.22:15002`
   （回滚 SQL：`/opt/ypbin/rollback-device-9300012-endpoint.sql`）并停掉模拟器。
2. **C13/C14/C15 的「299 / 47 / 60 / 12 点」早已失效**，且**与 access 无关**：那是**相对滑动窗口**的取值，
   而种子数据固定在 2026-09-25 04:24Z（CST 12:24）⇒ 窗口一滑动点数就会变。
   这些数字只能当作「当时那一刻的快照」，不要当断言用。
3. **`iot:latest:*` key 数不是链路判据**：实测 12 个 key 中包含 `iot:latest:1:9990001`/`9990002`
   两个**台账内不存在**的探针设备（`iot_device` 中 COUNT=0），它们与 access 链路无关。
...
| 读数（IoTDB 时序 + Redis 最新值 + `device_liveness` + 断档闭合） | **平台内部端点**：`POST /internal/readings`（`X-Internal-Token`） | 6 批共 **1394** 条（500+500+277+33+1+83），批批 `R.code=200`；这是**真业务路径** |
...
| B11 | 整窗维护 | `demo-dev-maint-full` | 可用率 **1.000000**、维护窗口 **79200s**、统计总时长仅 **7200s**、断档 0 次（分母被维护扣掉，这是「计划停机不该扣可用率」的可见证据）。⚠️ 该设备**故意没有任何读数**（所以「历史曲线」抽屉对它必然是空结果）；本场景的判据是 `effectiveWindowSeconds=7200`，不是 `availability=1.0`（后者在零数据下不具备区分度） |
...
> ⚠️ 下面的数字是**某一时刻的快照**（取数时刻见各条；采集于 2026-09-25 12:2x CST）。
> 可用率窗口是**滑动**的（默认「最近 24h」），且 `demo-dev-offline` 的断档是**进行中**，
> 因此它的 `outageSeconds/availability` 会随时间继续变化（实测 12:32:32 = 0.870764 → 12:35:51 = 0.868461）；
> 其余设备的断档窗口是固定的，数字稳定。
...
1. **同刻重复写（C17）**：对同一 `(device, point, ts)` 依次写 `false`（文本列）→ `1`（数值列）→ `2`（数值列）——IoTDB 表模型**不新增行**（总行数 **1394→1392**，少 2 行，全部来自这一个时刻：3 次写入合并成 1 行），但**不清除另一列**：该行最终 `value_double=2.0` 且 `value_text=false` 同时非空。
