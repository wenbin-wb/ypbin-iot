...
1. **问题不是「页面太少」，而是「后端能力已就绪、前端只暴露了其中 1/3」。**
   后端已有 `15` 个 IoT 控制器、`18` 张实体表；前端只做了 **4 个列表页 + 6 个抽屉组件（4 个表单抽屉 + 2 个功能抽屉）**，
...
> 本节由**独立复核子代理**产出（不同上下文、只读、自行跑命令与抓网页、不继承作者结论）。**共两轮。**
>
> - **第一轮：判定 FAIL** —— 三项承重结论经复核确认成立，但发现 **8 条不实/不精确断言**（详见 §11.1）。
>   **复核者同时确认：三项承重结论全部成立** ——
>   ①「复用现有接口」9 组断言与源码逐条一致；② 缺口 G1–G12 **全部 12 条**成立；
>   ③ 原型离线可开、无外链、可渲染（复核者自建 DOM 桩实跑 **45 次渲染、0 失败**）。
> - **第二轮（针对修正稿）：判定 PASS-with-issues** —— 第一轮 8 条**逐条实测均已真修**；
>   §0.3 的自我更正**完全属实且叙事可复现**；§9.5 所列 6 项「已就绪」接口**逐条在源码中找到**；
>   原型 `find()` 修复**实测生效**（无效 id 确实回落第一台，不再空白；复核者自建 DOM 桩跑 **104 次渲染 0 失败**）。
>   第二轮另发现 **1 条实质问题 + 4 条措辞/粘贴精度问题**，**亦已全部修正**（见 §11.6）。
...
| 6 | 次要 | 「4 个列表页 + **3 个抽屉**」与实测 6 个 `modules/` 组件口径不一 | 实测 6 个（4×form.vue + availability.vue + series.vue）；且 6 个**全部**基于 `useVbenDrawer`，其中**非表单的功能抽屉只有 2 个**（availability、series）——第二轮的精确化意见 | ✅ **已改为「4 个列表页 + 6 个抽屉组件（4 个表单抽屉 + 2 个功能抽屉）」**（第一轮写成「含 3 个功能抽屉」，第二轮指出「3」实为 `devices/index.vue` 注册的抽屉数，已纠正） |
...
| 8 | 提示 | 附录 A.3 的 G1 命令用 `grep -iv writer` 把 `RedisLatestValueWriterTest` 一并滤掉，**口径略糙** | 属实（该测试文件也命中 `iot:latest`），但**结论不变**（测试文件不构成读取接口） | ✅ **已改为三步精确命令**：①全量 grep `opsForHash`（看是否只有写）②按 Redis **读 API 正则**（`get\|entries\|multiGet\|keys\|values\|size\|hasKey`）确认无读 ③把范围收到 `controller/` 确认无读取端点。**新命令不按文件名过滤任何东西** |
...
复核者原判定 FAIL 的**全部 8 条问题均已修正**（其中第 2 条为「**如实并列冲突来源**」而非「改成单一值」——
阿里云两页 20/200 冲突，本方案不取其一）。
...
### 11.6 第二轮复核（针对修正稿）发现的问题与处理

第二轮判定 **PASS-with-issues**：第一轮 8 条**逐条实测均已真修**，仅剩 1 条实质问题 + 4 条精度问题。**均已修正**：

| # | 严重度 | 第二轮发现 | 处理 |
|---|---|---|---|
| R2-1 | **中等（新增不实）** | 附录 A.5 的自检写「表格数 **21**」，实测已是 **25** —— 因为本次修订**自己新增了 4 张表**（§0.3 更正表、§1.1 基线表、§9.5 清单表、§11.1 复核表）却**没同步更新这行输出** | ✅ **已改为 25**（并重跑脚本核对）。⚠️ **这正是本文档自己认定的失败模式：「贴出来的输出与真实不符」** —— 说明「自检脚本 + 粘贴输出」这种做法本身也会腐化，需要每次修订后重跑 |
| R2-2 | 次要 | §0.1「6 个模块组件（含 **3 个功能抽屉**）」括注不精确：6 个模块**全部**基于 `useVbenDrawer`，**非表单的功能抽屉只有 2 个**（availability、series）；「3」实为 `devices/index.vue` 注册的抽屉数 | ✅ 已改为「**6 个抽屉组件（4 个表单抽屉 + 2 个功能抽屉）**」 |
| R2-3 | 次要 | §11.1 第 8 行自述「只排除主类名」与新命令不符（新三步命令**完全不按文件名过滤**） | ✅ 已改写为对新命令的准确描述 |
| R2-4 | 次要 | §11.4「其中 **2** 条为『如实并列冲突来源』」与 §11.1 表格（只有第 2 条）对不上 | ✅ 已改为「其中**第 2 条**为…」 |
| R2-5 | 提示（粘贴不全） | 附录 A.3 的 G12 输出只贴了 2 行，真实有 4 行（另有 `006-iot-schema.sql:74` 段注释与迁移 DDL:17）；A.4 的 `grep 3204` 少贴第 155 行 | ✅ 均**已按真实完整输出补全**，并注明结论不变 |

**第二轮另做的两项独立加强验证（对本文有利，如实记录）**：
- 复核者用**自己的** DOM 桩跑了 **104 次渲染 / 0 失败**（比作者的 41 次组合更全）—— 支持「原型可渲染、不空白」的实质结论；
- 复核者**亲自 `web_fetch` 阿里云两页**，确认「英文 20 / 中文 200」的冲突**真实存在**，并确认 U11（`View Differences` 全流程）与 U12（Modbus 扩展信息字段表：`Operation Type` / `Register Address 0x0~0xFFFF` / `Original Data Type` / `Bit Position` / `Scale Factor` / 高低字节交换 / 寄存器位序交换 / `Data Report`）**在英文页原文可查** ⇒ U11/U12 的英文页来源成立。

**第二轮未能核实（与 §10.1 一致）**：活库 `sys_menu` 真实状态（无凭据未连库）；ThingsBoard 全部主张（未重新抓取；已在 U10 降级为旁证）；§2 其余厂商逐条细节本轮未逐页重访。

**当前状态**：两轮复核的全部问题已修完；本轮**未再引入新断言**（R2-1 之后未新增表格）。
...
./deploy/sql/006-iot-schema.sql:74:-- 设备台账扩展（§4.1：product_id/product_version/credential_ref/online_status/shadow_json/last_seen_at）
./deploy/sql/006-iot-schema.sql:80:    ADD COLUMN shadow_json     TEXT         NULL COMMENT '影子快照（§3.10）',
./deploy/sql/migration/2026-09-19-iot-m1-thing-model-schema.sql:17:    ADD COLUMN shadow_json     TEXT         NULL COMMENT '影子快照（§3.10）',
./ypbin-service-api/.../entity/IotDevice.java:68: private String shadowJson;
# ⇒ 4 处命中：1 处段注释 + 2 处 DDL + 1 处实体字段，**没有任何读写代码** ⇒ 「死列」成立
#   （注：第二轮复核者指出本文初版此处只贴了 2 行，已按真实完整输出补全；结论不变）
...
155:--   （当时在用的 32xx 只有 3200-3203 与 320001-320017），仓内 SQL 亦无 3204。
...
172:-- 把 4 个 IoT 页面挂到新父菜单下（pid 由 0 改为 3204）；按钮权限因挂在页面下，层级随之自动下沉。
...
```bash
$ python3   # 统计每张表的每行列数（区分转义 \|），并核对引用编号双向一致
列数不一致: 0                 # ← 这一项是硬断言，必须为 0
引用了但清单缺失: []
清单有但正文未引用: []
```

> ⚠️ **这里故意不写「表格数 = N」**。本文档曾两次因为**修订时新增了表却忘了同步更新这行计数**而被独立复核判为「贴出的输出与真实不符」
> （第一轮发现 21→实际更多、第二轮发现 21→25，第二轮之后又因新增 §11.6 表变成 26）。
> **教训**：凡是把「脚本输出」粘进文档，就要接受它每次修订都可能腐化 ——
> 对**会随修订变化的数字**（表格数、行数）不应固化进正文；**只固化真正的不变量**（`列数不一致 = 0`、引用双向一致、外链数 = 0）。
> 需要具体计数时，**现场重跑脚本**即可（脚本逻辑见本附录各节）。
