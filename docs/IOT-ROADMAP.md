...
| # | 事项 | 现状 |
|---|---|---|
| **A1** | ~~access 侧上报接线~~ **已落地**（`HttpAccessReadingSink`）：有界队列 → 微批 → `/internal/readings`，含丢弃/失败/非法计数与超时；EMQX 传输**待实施**（Q4 已代决：内置库认证 + REST，见设计文档 D0.6） | 已收口；仍有限制见 A9/A10 |
...
| **A2** | 读数**值**不落库、Redis 最新值未做 | 依赖 Q8 —— **已代决**（2026-09-23，见设计文档 D0.7）：**IoTDB 2.x 表模型 + Redis 7 最新值**；实施顺序见本行后续增量（接入总线先按 D0.6 切 EMQX，再写存储） |
