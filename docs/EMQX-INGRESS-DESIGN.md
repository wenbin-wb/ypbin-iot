...
>
> ✅ **用户已于 2026-09-26 就三项争议拍板**（入站路径 / 破例范围 / 资源策略）——见 **§1.1 决策记录**。
> 该记录是本文档的**约束**：与本节结论冲突时以 §1.1 为准（本文档已按决策同步）。
...
|---|---|---|---|
| **D1 · 入站路径** | **采用主推**：EMQX Rule Engine + HTTP Server 动作 → 薄适配端点。**否掉备选 B**（原样复用 `/internal/readings`）。备选 A（access 自建 MQTT 消费者）**降为备选、不实施**。 | 「**平台宕机不丢数据**比吞吐优势更值」：主推路径的重试与缓冲在 broker 侧（`max_retries=2`/`request_ttl=45s`/`max_buffer_bytes`）；备选 A 在平台宕机时只能靠 broker 持久会话兜，风险更高；备选 B 会因 200 语义错配**静默丢报文**，直接否掉。 | ① §6.1 为主推、§6.3 标注为「按 D1 不实施、仅保留设计」；② **覆盖 `docs/IOT-ROADMAP.md` 的旧倾向**（该文相应行已标注「已被本设计取代」）；③ §4.2 的取舍结论与之一致（本次决策即该节评审结论）。 |
...
| **D3 · 资源策略** | **采用保守路线**：**拿到 §8.3 阶段② 的 OOM/存活实测数据之前，不把 EMQX 与其入站链路一起上生产**。要推进就按**降级形态**：EMQX 只做**认证/ACL + 下行/在线调试**，入站继续走既有 HTTP 通道。 | 生产机 available≈904MB、swap=0、磁盘已用 92%；**官方未给出最低内存要求** ⇒ 不能把余量当够用。 | ① §9 的 P0 **重排为两组**（P0-A 不依赖 EMQX / P0-B 等实测），见下；② §8.3 的三选项给出**代价与建议顺序**；③ 降级形态成为**默认首发形态**。 |
...
4. **端点必须有 `propertyId` 白名单/长度校验与 `requestId` 幂等**。→ 白名单见 §6.5 落点说明 + P0-A-2；`requestId` 幂等属下行命令实例（P0-A-4）。
5. **在 `docs/IOT-PLATFORM-DESIGN.md`（或专门文档）登记这条唯一例外**，便于门禁与后来者识别。→ 已登记于该文 §13 内部端点表 + 新增「唯一例外」说明。
...
| 序 | 项 | 为什么放这里 |
|---|---|---|
| **P0-A-1** | 🔴 **最新值乱序防护**（跨批 ts 比较；含「新 ts 先到、旧 ts 后到」用例 + 变异验证） | 与 EMQX 无关：既有 HTTP 通道已存在该风险；EMQX 入站只会放大它（§6.4） |
...
| **P0-B-2** | 入站：规则 + HTTP 动作 + **薄适配端点 `POST /internal/mqtt/readings`**（含破例的 401/400 实现与单测） | P0-A-1/2/5 已完成；D2 的 5 条要求逐条满足 |
...
### 6.3 备选路径（P1，**按 D1 决策不实施，仅保留设计**）：`ypbin-access` 自建 MQTT 消费者

> ⚠️ **本节不进入实施计划**（用户 2026-09-26 决策 D1）：主推路径的「平台宕机不丢数据」优于本节的吞吐/背压优势。
> 只有主推路径被证伪时才重启本节（P0-B-5）。下面内容作为**备选设计**保留，便于将来评估。
