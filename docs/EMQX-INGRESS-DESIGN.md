...
| # | 结论 | 强度 |
|---|---|---|
| **C1** | **入站主推：EMQX Rule Engine + HTTP Server 动作 → 新增薄适配端点 `POST /internal/mqtt/readings`（同 `X-Internal-Token` 守卫）→ 复用既有 `AvailabilityService.ingest` 及其后的最新值/时序/活性链路。** 规则 SQL 把一条上行消息规格化成 `ReadingIngestReq` 的**单元素 `items`**。**新增的只有一个薄适配控制器**（职责只有两件：契约适配 + **非法报文返回非 2xx**，见 §6.5/C9/H10），**不新增任何落库链路**，既有 `ingest()` 与 `POST /internal/readings` 的契约与语义**一行不改**。备选 A：`ypbin-access` 自建 MQTT 消费者复用既有有界队列微批；备选 B：**零 Java 改动**（原样复用 `/internal/readings`），代价是非法报文被 EMQX 判成功而静默丢弃，只能在规则 SQL 里做有限前置校验（§6.5 方案 B）。 | 已核实（官方文档 + 官方源码 + 仓内代码） |
...
| **C11** | ✅ **三项争议已由用户 2026-09-26 拍板**：D1 入站路径（取主推）、D2 破例范围（仅 `/internal/mqtt/**`）、D3 资源策略（默认降级形态）。**见 §1.1 决策记录；本文档已按决策同步**（与本节其它结论冲突时以 §1.1 为准）。 | 用户决策（非本文档推断） |
...
- **既有缺口的现状（核对 main 后更正）**：**G2（影子 `reported`）已在 main 落地**（PR #46，且已接入本设计复用的 `ingest` 链路，见 §7.3）；仍在另立批次的是 **§6.4 最新值乱序防护**（P0-A-1，**另有并行批次正在处置；截至本提交其改动尚未进入 main**）与 **G4 点位映射挂产品**（P2-1）。

---

### 1.1 决策记录（用户 2026-09-26 拍板）

> 三条决策由**用户**做出（非本文档自行推断），记录在此以便「后人/门禁/评审」有唯一出处。
> 每条 = 决策 + 理由 + **对本文档与其它文档的实际影响**。

| 编号 | 决策 | 理由（用户口径） | 落地影响（本文档/它文） |
|---|---|---|---|
| **D1 · 入站路径** | **采用主推**：EMQX Rule Engine + HTTP Server 动作 → 薄适配端点。**否掉备选 B**（原样复用 `/internal/readings`）。备选 A（access 自建 MQTT 消费者）**降为备选、不实施**。 | 「**平台宕机不丢数据**比吞吐优势更值」：主推路径的重试与缓冲在 broker 侧（`max_retries=2`/`request_ttl=45s`/`max_buffer_bytes`）；备选 A 在平台宕机时只能靠 broker 持久会话兜，风险更高；备选 B 会因 200 语义错配**静默丢报文**，直接否掉。 | ① §6.1 为主推；§6.3 与 §9 P1-1 标注「按 D1 不实施」；② **覆盖 `docs/IOT-ROADMAP.md` 的旧倾向**（该文相应行已标注「已被取代」）；③ §4.2 的取舍结论即该节评审结论（已拍板）。 |
| **D2 · 破例范围** | **批准为入站适配端点破例**：**仅** `/internal/mqtt/**` 这类**机器对机器**的入站适配端点可返回**真 HTTP 状态码**（非法报文 4xx、可重试 5xx），**不适用**「全局 HTTP 200 + `R.code`」惯例。 | 对端是 EMQX 的连接器，**只看状态码**（官方源码：`emqx_bridge_http_connector.erl:963-986`）；不破例 ⇒ 非法报文被判成功、不重试、不计入 `dropped.*` ⇒ **双端都绿但数据不存在**。 | ① §6.5 的「方案 A」由**建议**升为**已批准**；② 必须满足 5 条附带要求（见下）；③ 该例外登记在 `docs/IOT-PLATFORM-DESIGN.md` §13 内部端点表（便于门禁识别）。 |
| **D3 · 资源策略** | **采用保守路线**：**拿到 §8.3 阶段② 的 OOM/存活实测数据之前，不把 EMQX 与其入站链路一起上生产**。要推进就按**降级形态**：EMQX 只做**认证/ACL + 下行/在线调试**，入站继续走既有 HTTP 通道。 | 生产机 available≈904MB、swap=0、磁盘已用 92%；**官方未给出最低内存要求** ⇒ 不能把余量当够用。 | ① §9 的 P0 **重排为两组**（P0-A 不依赖 EMQX / P0-B 等实测），见下；② §8.3 阶段③ 给出**代价与建议顺序**；③ 降级形态成为**默认首发形态**。 |

**D2 的 5 条附带要求（用户明确要求「范围与理由必须钉死」）**

1. **在端点 Javadoc 与文档里写明为什么破例**（对端是 EMQX 连接器、只看状态码；不破例 ⇒ 非法报文被判成功、不重试、不计入 `dropped.*` ⇒ 双端都绿但数据不存在）。→ 本文档 §6.5 已写；实施时**必须**同样写入该类 Javadoc。
2. **浏览器/网关面向的 API 一律维持「HTTP 200 + `R.code`」惯例，不得扩散**。→ 破例的作用域在实现上必须是 `/internal/mqtt/**`（**不放行 `/internal/**` 全体**，既有 `/internal/readings`、`/internal/lease/**` 等一律维持原惯例）。
3. **非法报文必须在「动库之前」整批拒绝**，并计入 EMQX 侧可观测指标（重试计数 / `dropped.*`）。→ §6.5 验收口径第 1 条（`count(*)` 前后不变）。
4. **端点必须有 `propertyId` 白名单/长度校验与 `requestId` 幂等**。→ 白名单见 §6.5 落点说明 + P0-A-2；`requestId` 幂等属下**行命令实例**（P0-A-4，`iot_command_instance.request_id` 唯一键 + 回执 CAS）。
5. **在 `docs/IOT-PLATFORM-DESIGN.md`（或专门文档）登记这条唯一例外**，便于门禁与后来者识别。→ 已登记于该文 §13 内部端点表 + 「唯一例外」专条。

### 1.2 P0 执行顺序（按 D3 重排：先做不依赖 EMQX 的，后做等实测的）

> 顺序是**约束**：**P0-A 全部完成并通过验收后**才启动 P0-B 的容器侧工作；P0-B 的资源实测不通过 ⇒ 停在**降级形态**（EMQX 只做认证/ACL + 下行），P0-A 的成果在降级形态下**全部仍然有效**（这也是把它们排在前面的原因）。

**P0-A · 不依赖 EMQX，立即可做（降级形态下也用得上）**

| 序 | 项 | 为什么放这里 |
|---|---|---|
| **P0-A-1** | 🔴 **最新值乱序防护**（跨批 ts 比较；含「新 ts 先到、旧 ts 后到」用例 + 变异验证） | 与 EMQX 无关：既有 HTTP 通道已存在该风险；EMQX 入站只会放大它（§6.4）。**注：另有并行批次正在处置（截至本提交其改动未进 main）** |
| **P0-A-2** | 🔴 **`propertyId` 白名单 + 长度校验**（物模型属性 × 该设备点位映射） | 入站输入不可信铁律；HTTP 通道与 MQTT 通道共用 |
| **P0-A-3** | **凭据 4 端点 + `iot_device` 加 3 列**（签发/查看元数据/吊销/接入信息 + 级联删除） | 纯平台侧；下行与降级形态都要用（G10） |
| **P0-A-4** | **命令实例表 + 状态机 + 超时扫描 + 回执端点 + 权限码/菜单** | 纯平台侧；**降级形态的核心交付**（下行/在线调试可用） |
| **P0-A-5** | **EMQX REST 客户端**（签发用户、publish；显式超时、可区分错误码） | 纯 Java；可先用 mock/单测验收，等 EMQX 到位即可联调 |

**P0-B · 必须等 §8.3 阶段② 资源实测（不通过则停在降级形态）**

| 序 | 项 | 前置 |
|---|---|---|
| **P0-B-1** | EMQX 容器 + 卷 + `emqx.conf` + `emqx-init.sh` + Nacos 配置 + `mem_limit`/`max_buffer_bytes` | 磁盘/内存实测通过（阶段①②） |
| **P0-B-2** | 入站：规则 + HTTP 动作 + **薄适配端点 `POST /internal/mqtt/readings`**（含破例的 400/401 实现与单测） | P0-A-1/2/5 已完成；D2 的 5 条要求逐条满足 |
| **P0-B-3** | 设备侧端到端联调（真实 MQTT 设备发 `up/property`） | P0-B-1/B-2 |
| **P0-B-4** | TLS(8883) 与对外暴露面评审 | P0-B-3 通过 |
| **P0-B-5** | 备选 A（access 自建消费者）—— **按 D1 不实施**，仅在主推路径被证伪时重启 | D1 被推翻 |

**三选项的代价与建议顺序（D3；与 §8.3 阶段③一致）**

| 顺序 | 选项 | 代价 | 何时选 |
|---|---|---|---|
| **① 首选** | **降级形态**：EMQX 只做**认证/ACL + 下行/在线调试**，入站继续既有 HTTP 通道 | **低风险、立即能用**；代价是设备侧可能需双通道（HTTP 上报 + MQTT 下行） | 默认首选；也是 P0-B 实测不通过时的**落点** |
| **② 次选** | **腾内存**：下调 Nacos / IoTDB 内存上限 | **需用户批准**（nacos 1.46GiB 是注册+配置中心，压缩它会影响**已在跑的服务**），风险外溢到其它容器 | 降级形态无法满足业务（必须单通道 MQTT 入站）时 |
| **③ 末选** | **另机部署 EMQX** | 成本最高：跨机网络抖动与延迟进入数据面、新的部署与监控；入站端点要么平台内网可达（需评估公网暴露），要么改用备选 A（已按 D1 否掉） | ①② 都不成立时 |
...
> - 本文档**不集中列举**「哪些条已被独立复核」——集中列举会与行级标注漂移（首版曾出现「声称 19 条、实列 22 项」且与行内标注冲突的问题，已删除）。**一律以每条目的行内标注为准**：
>   - 行内写 **（独立复核 2026-09-26 逐字确认）** 或含「**独立复核**」字样的 ⇒ 已被第二轮外委复核者亲自取回官方原文/原始 HTML/官方源码逐字确认；
...
| F19 | 主题占位符为 `${username}` / `${clientid}` / `${client_attrs.NAME}`，且**可以只占主题的一段**：*Placeholders can be used as topic segments, like `a/b/${username}/c/d`*。**本轮未检索到 `%u`/`%c` 语法** | 官方原文（子代理一手核实） | [authz/authz.html](https://docs.emqx.com/en/emqx/v5.8/guides/access-control/authz/authz.html) （**独立复核 2026-09-26 逐字确认**） |
| F20 | 两级动态主题（每设备只能收发自己的 up/down）需要 `client_attrs`：`mqtt.client_attrs_init = [{expression="nth(1, tokens(username, '.'))", set_as_attr=...}, …]`；官方示例正是 `up/${client_attrs.productId}/${client_attrs.deviceId}` | 官方原文（子代理一手核实；表达式函数 `nth`/`tokens` 逐字来自官方示例） | [client-attributes.html](https://docs.emqx.com/en/emqx/v5.8/develop/client-attributes/client-attributes.html) （**独立复核 2026-09-26 逐字确认**） |
| F21 | 🔴 官方安全要求：ACL 主题模板里用 `${clientid}`/`${username}`/`${client_attrs.X}` 时**必须校验这些身份值不含 MQTT 通配符 `+`/`#` 与主题分隔符 `/`** | 官方原文（子代理一手核实） | [security-checklist.html](https://docs.emqx.com/en/emqx/v5.8/guides/access-control/security-checklist.html) （**独立复核 2026-09-26 逐字确认**） |
...
| F33 | 响应码：`200` *delivered to at least one subscriber*；**`202` *No matched subscribers***；`400` invalid；`503` 投递失败 | 官方 OpenAPI 原文（子代理一手核实） | 同上 （**独立复核 2026-09-26 逐字确认**） |
| F34 | 🔴 **没有「按 clientid 定向发布」的端点**（OpenAPI 193 条路径中无此端点；`clientid` 字段已 deprecated） | 否定性核实（子代理一手核实） | 同上 （**独立复核 2026-09-26 逐字确认**） |
...
| # | 事实 | 原文/证据 | 出处 |
|---|---|---|---|
| F38 | Dashboard 默认 `admin`/`public`，**首次登录强制改密**；生产前必须改密 | 官方原文 | [dashboard/introduction.html](https://docs.emqx.com/en/emqx/v5.8/guides/dashboard/introduction.html)、[security-checklist.html](https://docs.emqx.com/en/emqx/v5.8/guides/access-control/security-checklist.html) （**独立复核 2026-09-26 逐字确认**） |
...
flowchart TB
    ...
    subgraph PLAT["平台（ypbin-net）"]
        IOT["ypbin-iot :18084<br/>POST /internal/mqtt/readings<br/>（新增薄适配端点：契约适配 + 非法即 4xx）"]
    ...
    AVAIL --> R2
    AVAIL -- "desired 由命令/影子写入；<br/>reported 由 ingest 在 afterCommit 同步（G2 已落地）" --> SHD
...
|---|---|---|
| 平台代码量 | **很小**：规则 + 桥接配置 + **1 个薄适配控制器**（契约适配 + 非法即 4xx，复用既有 `ingest` 与全部落库链路） | 较大：需在 access 增加 MQTT 客户端 + 租约联动订阅/退订 + 断线重连 + 测试 |
...
| **结论** | **P0 选它，且已由用户决策 D1 定为现行方案**：先把链路端到端跑通，代价是每消息一次 HTTP（本批验收含吞吐压测，见 §8.3） | ⚠️ **按 D1 不实施（降级为备选，仅保留设计）**：它把吞吐/背压收回到仓内既有纪律，但对「平台宕机不丢数据」不如主推路径；也是批量属性上报的潜在路径 |

> ✅ **本节的取舍已由用户决策 D1 拍板（2026-09-26，见 §1.1）**：取主推、备选 A 不实施。
>
> R8（保留决策依据）：这条与 `docs/IOT-ROADMAP.md:743` 的**旧倾向**（*EMQX 5.x … + iot 侧共享订阅消费者*）**不一致**（该行已标注「已被取代」）。改判理由只有一条实测事实：**本机 available ≈ 904MB、swap=0**，而平台自建消费者在平台重启窗口内的消息只能靠 broker 的持久会话兜（默认队列 1000 条、Durable Sessions 默认关闭且是企业版）。Rule Engine 路径把这段窗口的**缓冲责任留在 broker 内存**，对「设备一断电就长时间离网」的现场更稳。**该取舍请随本 PR 一并评审**，若结论相反，把 §6.3 升为 P0 即可，§4–§5、§7–§9 的其余设计不受影响。
...
- 服务侧：`AvailabilityServiceImpl.ingest`（`@Transactional(rollbackFor = Exception.class)`）→ 活性/断档 + `afterCommit` 写最新值/时序 —— `.../service/impl/AvailabilityServiceImpl.java:120-146`（`writeDerivedAfterCommit` 在 `:220`，实际写调用在 `:246`）
...
- **多属性一条消息不在 P0**：`items` 的展开需要服务端或规则引擎做循环，而 HTTP 动作的 body 模板**不支持数组展开**（**该点未核实**，见 §10.2 U24；此处按最保守假设设计） ⇒ 多属性请发多条，或等 §6.3 备选路径在主推被证伪后重启（P0-B-5，见 §1.1 D1）时，由自建消费者自行展开数组。这是主推路径的真实代价，写在这里而不是留给实施者踩。

**桥接配置要点（每一条都有事实依据）**

| 键 | 建议值 | 依据 |
|---|---|---|
| `url` | `http://ypbin-iot:18084/internal/mqtt/readings` | 同 `ypbin-net`，容器名即服务名（`docker-compose.yml` 中 ypbin-iot 有固定 IP `172.20.0.24` 与网络别名）；`url` 的 scheme/host/port 不能用模板（F24） |
| `method` | `post`（默认） | F24 |
| `headers` | `content-type: application/json` + `X-Internal-Token: <deploy/.env 的 INTERNAL_TOKEN 实际值>` | 守卫头名见 `InternalTokenGuardInterceptor.java:62`；header 支持模板（F24）。⚠️ **取值来源**：`deploy/nacos/ypbin-common.yaml:75-76` 的 `ypbin.internal.token: ${INTERNAL_TOKEN}` 由 `deploy/install.sh` 在导入 Nacos 前替换为 `deploy/.env` 的实际值（**生成值**在 `:1093`，**Nacos 占位符替换的 sed** 在 `:1288-1291`） ⇒ 桥接里**必须填替换后的同一个值**，不能写字面量 `${INTERNAL_TOKEN}` |
...
| 批量 | P0 无批量（1 msg = 1 HTTP）；原计划 **P1** 走备选路径获得 200 条/批 —— ⚠️ **该备选已按 D1 定为不实施**，故批量能力**当前无落地计划**（若将来需要，见 §4.2 的重新评估条件） | §1.1 D1 / §4.2 |
...
### 6.3 备选路径（**按 D1 决策不实施，仅保留设计**）：`ypbin-access` 自建 MQTT 消费者

> ⚠️ **本节不进入实施计划**（用户 2026-09-26 决策 D1）：主推路径的「平台宕机不丢数据」优于本节的吞吐/背压优势。
> 只有主推路径被证伪时才重启本节（P0-B-5）。以下内容作为**备选设计**保留，便于将来评估。
...
| 方案 | 做法 | 取舍 |
|---|---|---|
| **A（推荐·**已按 D2 批准**）· 入站专用端点** | 新增 `POST /internal/mqtt/readings`（同守卫），**校验失败即返回非 2xx**（`400`），成功才 2xx；失败细节进日志与指标 | 需要在 `/internal/**` 里**破例**一个「不遵全局 HTTP 200」的端点 ⇒ 必须在代码注释与本文档写明这是**协议边界**的刻意例外，并加一个架构门禁/单测断言它确实返回 400 |
...
**结论**：**A + B 组合**。A 是 P0（否则 H10 不达标），B 作为纵深防御。**其中 A 已由用户批准为「唯一例外」**（决策 D2，作用域与 5 条附带要求见 §1.1），B 仍只是纵深防御、不替代 A。**若选 A，`/internal/readings` 的既有契约与语义一行不改**（HTTP 上报通道继续返回 HTTP 200，两侧语义差异由「MQTT 入站专用端点」承担）——这也是「复用既有落库链路」的正确含义：**复用服务方法，不复用它的 HTTP 信封语义**。
...
> ⚠️ **更正记录（2026-09-26）**：本节此前写「G2 未完成、`reported` 无写入方、`merged ≡ desired`」——那是本文档写作时（基于当时分支）的事实，**在 main 上已不成立**（#46 已落地并接入 `ingest`）。保留更正痕迹以免读者据旧结论做判断。
>
> R8（仍然成立）：**不要**让「命令成功」依赖 `reported`。命令状态由**回执**决定（`up/reply`），影子一致性是**另一条**展示轴（设备是否如实上报了期望值）。两者混在一起会把「设备不报数」误判成「命令失败」。G2 落地前，UI 必须如实标注 `reported` 为空是**能力缺失**而不是「设备没上报」。
...
> **建议顺序（用户 D3 决策）**：**① 降级形态（低风险、立即能用） ＞ ② 腾内存（需用户批准降 nacos/iotdb，风险外溢） ＞ ③ 另机部署（成本与跨机延迟最高）**。
> 降级形态 = **EMQX 只做认证/ACL + 下行/在线调试，入站继续走既有 HTTP 通道**；它是**默认首发形态**，也是 P0-B 实测不通过时的落点（P0-A 的成果在降级形态下全部有效）。

| 顺序 | 选项 | 做法 | 代价/风险 |
|---|---|---|---|
| **② 次选** | **腾内存** | 下调 Nacos / IoTDB 的内存上限（IoTDB 已有 `MEMORY_SIZE=768M` 覆盖先例） | 影响**已在跑的业务**（nacos 1.46GiB 是注册中心 + 配置中心，压缩它可能影响现有服务）；需逐个实测，风险外溢到别的容器 ⇒ **需用户批准** |
| **③ 末选** | **另机部署 EMQX** | EMQX 放到另一台机器，平台侧通过内网/公网连 broker | 最稳（不动现有栈）；代价：跨机网络抖动与延迟进入数据面、需要新的部署与监控、入站端点的调用要从 EMQX 跨机回平台（**要么平台内网可达（需评估公网暴露）**，要么改用备选路径由 access 主动外连 broker）；成本与运维面上升 |
| **① 首选** | **降级形态**（用户 D3 批准的默认首发形态） | EMQX **只做认证/ACL + 下行/在线调试**，入站继续走**既有 HTTP 通道**（`/internal/readings`，已上线验证） | **低风险、立即能用**；代价是设备侧**双通道**（HTTP 上报 + MQTT 下行）⇒ 设备侧复杂度上升；**P0-A 的全部成果在降级形态下仍然有效** |

> 明确表态（R4）：**在拿到阶段 2 的实测数据之前，本方案不建议把 EMQX 与其入站链路一起上生产。** 若必须推进，选 **① 的降级形态**（顺序见 §1.2）（EMQX 只做下行 + 认证/ACL，入站继续 HTTP）。
...
> 标注：**[前端]** / **[后端]** / **[中间件]** / **[数据]**
> 每项都给**验收口径**（状态码 / 行数 / 可复现步骤）。SQL 一律**双写**：等价语句追加到 **`007-iot-data.sql` 末尾**（❌ 不是 `006`）+ `migration/2026-09-28-iot-emqx-ingress.sql`（文件名须含 `-iot-`），否则 `tools/check-iot-sql-equivalence.sh` 会红（理由见附录 A）。

### P0（本轮之后的第一批实施）

> ⚠️ **执行顺序以 §1.2 为准（用户 D3 决策）**：P0 分两组 —— **P0-A（不依赖 EMQX，立即可做）** 与 **P0-B（必须等 §8.3 阶段② 资源实测；不通过则停在降级形态）**。
> 下表沿用编号（便于与复核记录对齐），映射：
> `P0-A-1`=P0-6（乱序防护）｜`P0-A-2`=P0-6c（`propertyId` 白名单）｜`P0-A-3`=P0-3（凭据 4 端点 + 3 列）+P0-4（EMQX REST 客户端，可先单测）｜`P0-A-4`=P0-7（命令实例表/状态机/超时扫描/回执）+P0-8（权限码与菜单）；
> `P0-B-1`=P0-1（容器/卷/HOCON/init/Nacos）+P0-2/P0-2b（资源与磁盘实测）｜`P0-B-2`=P0-5（入站规则 + 薄适配端点）+P0-6b（失败语义 4xx/401）｜`P0-B-3`=P0-9（前端联调）。
...
| P0-5 | **入站：规则 + HTTP 动作 → `POST /internal/mqtt/readings`**（薄适配控制器 + 中间件配置）+ `up/property` 契约 | [中间件][后端] | 端到端：用测试设备发 1 条 `up/property` → `GET /iot/devices/{id}/latest` 返回该点位（HTTP 200）；`SELECT count(*) FROM device_liveness WHERE device_id=?` = 1；IoTDB `reading` 表新增 1 行 |
...
| # | 项 | 类型 | 验收口径 |
|---|---|---|---|
| ~~P1-1~~ | ~~**备选入站路径**：access 自建 MQTT 消费者（§6.3）~~ —— ⚠️ **按 D1 决策不实施**（仅保留设计；重启条件见 §1.2 P0-B-5） | [后端] | 停掉 HTTP 上报、只留 MQTT，读数仍进最新值/时序；`iot.access.egress.dropped` 在压测下不异常增长；租约转移后**只有新节点**在收（旧节点订阅已退订） |
...
| ~~P2-2~~ | ~~G2：影子 `reported` 写入接入入站链路~~ —— ✅ **已在 main 落地（PR #46）**，不再是待办 | [后端] |
...
-- 沿用既有 id 段：3200 设备菜单下挂按钮；**3240 段**是 IA 方案的「④ 数据与调试」分组（见
-- docs/PLATFORM-IA-PROPOSAL.md：3210=① 接入配置 / 3240=④ 数据与调试），该分组菜单尚未落地，
...
- **G2（影子 `reported`）**：**已在 main 落地（PR #46）**，且已接入本设计复用的 `ingest` 链路 ⇒ 本方案不再需要为它做任何 Schema 改动（§7.3）。
...
| `docs/IOT-ROADMAP.md:743` | 「iot 侧共享订阅消费者」 | 主推 Rule Engine 入站、备选 access 自建消费者（§4.2） | ✅ **已执行**：该行已标注「已被取代（2026-09-26 决策 D1）」；决策见 §1.1 |
...
## 资源决策记录：EMQX **不在本机部署**（2026-09-26 决定）

**背景（实测）**：本机 `available` 长期仅 **379–393 MB**、**无 swap**、根盘 87%；
`ypbin-iot` 903.7MiB + `ypbin-access` 648.8MiB 常驻；滚动重启窗口内 load 一度冲到 **86**。

**决定**：
1. **EMQX 不在该生产机上部署**——改为**另机部署**，或**等腾出内存后再评估**（二选一，未定）。
2. **硬约束**：**不得再起任何新容器/中间件**；`ypbin-access` 必须保留（采集链路在用）。
3. 若后续操作需要更多内存，**先停下报告**，并给出三选项与代价：
   - **降堆**（nacos / iotdb 的 `MEMORY_SIZE` 或 JVM 堆）—— 代价：中间件余量变小，压力下更易 OOM；
   - **停非必需服务** —— 代价：功能缺失（须明确停哪些、影响谁）；
   - **另机** —— 代价：跨机网络/延迟与运维面增加（EMQX 场景即此选项）。

**为什么钉在设计文档里**：EMQX 入站通道是本设计（F1–F45）的载重项，其可行性**首先受本机内存约束**，
而不是协议选型；不写清这条，后续很容易again按「同机加一个容器」去排期。
