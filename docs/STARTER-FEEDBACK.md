...
wc -l docs/STARTER-FEEDBACK.md; python3 - <<'PY'
r='docs/IOT-ROADMAP.md'
t=open(r,encoding='utf-8').read()
anchor = "### 五、替换缝（3a 已备好，3b-2 只需新增自动配置）"
assert t.count(anchor)==1
extra = """### 四点十八、反哺 starter 的需求清单（2026-09-24，交接材料）
...
print('roadmap 四点十八 ok')
PY
git add -A && git commit -q -F - <<'EOF'
docs(iot): 新增反哺 starter 的需求清单（SF-1~SF-3）并登记 ROADMAP 四点十八

用户要求「有需要反哺 starter 的功能要记着，我会新开会话让它去改」⇒ 整理成**自包含交接文档**
`docs/STARTER-FEEDBACK.md`（那边新开会话不必先读 ypbin-iot 上下文），并在 ROADMAP 新增四点十八 索引表。
每条含：现象 / 证据（外委复核的探针输出与一手来源）/ 影响 / 期望能力（含推荐方案）/ 验收标准 /
**会被替换掉的临时实现**，以及关闭约定。

- **SF-1（高｜安全）**：微服务下游 `@SaCheckPermission` 实际不生效——注解鉴权与登录拦截被
  `ypbin.security.interceptor` 一个开关绑死（下游无 Sa-Token 会话只能关它 ⇒ 注解鉴权一并失效）。
  期望：A 以 `IdentityContext` 为基准的注解鉴权拦截器（推荐）或 B 拆分两个开关；验收含「无会话仅有身份头
  也能正确判定」+「超管 `*:*:*` 通配必须保留」+「本仓删除 `IotPermissionGuard` 后仍全绿」。
- **SF-2（中）**：`@Idempotent` 默认键用 `Arrays.deepHashCode(args)`，而本仓 DTO 规范禁止值语义 equals
  ⇒ 带 Req 的写端点幂等形同虚设（探针实测同内容不同键，`Long` 参数正常）。
- **SF-3（低｜DX）**：`LoginUser` 字段名 `id`（非 `userId`）、与 `IdentityContext` 分属两个包易 import 错。
- 附 **UP-1**：admin 的 Sync Whitelist 不覆盖 `.github/**` ⇒ 下游只能新增 workflow 文件，不能统一加固 CI。
